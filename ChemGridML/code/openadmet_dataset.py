...
class OpenADMETDataset:
    ...
    def _load_data(self):
        """Load data from CSV and generate features"""
        # Read CSV
        ...
        smiles = df[self.smiles_column].values
        ...
        if self.X is None or (isinstance(self.X, np.ndarray) and self.X.size == 0):
            ...
            # Save to cache (avoid caching object arrays that may not serialize well)
            try:
                if not isinstance(self.X, np.ndarray) or self.X.dtype == object:
                    # Skip caching GRAPH/objects
                    pass
                else:
        ...
        # Convert empty strings and NaN to np.nan
        ...
        # Filter to only non-missing values
